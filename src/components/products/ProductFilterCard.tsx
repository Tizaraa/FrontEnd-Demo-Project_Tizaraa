...
type ProductFilterCardProps = {
  onBrandChange: (brands: number[]) => void;
  onCategoryChange: (categories: number[]) => void; // Changed to match first code
  onCountryChange: (countryIds: number[]) => void;
  onProvinceChange: (provinces: number[]) => void;
  slug: string;
  pageType?: string;
...
const ProductFilterCard: React.FC<ProductFilterCardProps> = ({
  ...
  const [provinceList, setProvinceList] = useState<Province[]>([]);
  const [selectedBrands, setSelectedBrands] = useState<number[]>([]);
  const [selectedCategories, setSelectedCategories] = useState<number[]>([]); // Added for categories
  const [selectedCountry, setSelectedCountry] = useState<number[]>([]);
  const [selectedProvinces, setSelectedProvinces] = useState<number[]>([]);
    ...
    <div style={{ marginLeft: level > 0 ? '16px' : '0', display: 'flex', flexWrap: 'wrap', gap: '8px' }}>
      {items.map((item) => (
        <React.Fragment key={item.id}>
          ...
          />
          {item.child && item.child.length > 0 && renderCategories(item.child, level + 1)}
        </React.Fragment>
      ))}
    </div>
  );
  ...
  // Flatten categories for show more/less functionality
  const flattenCategories = (categories: Category[]): Category[] => {
    return categories.reduce<Category[]>((acc, category) => {
      return [
        ...acc,
        category,
        ...(category.child ? flattenCategories(category.child) : [])
      ];
    }, []);
  };
