...
export default function GroceryProducts() {
 const { data } = useFetcher(`frontend/category/product/view`);
 const groceryProducts = data?.GroceryProduct || [];
 ...
 const loopedProducts = [...groceryProducts, ...groceryProducts];
 ...
 useEffect(() => {
  if (groceryProducts.length === 0) return;

  if (!isPaused) {
   intervalRef.current = setInterval(() => {
    setCurrentSlide((prev) => (prev + 1) % groceryProducts.length);
   }, 2000); // Slide every 3s
  }
 ...
 }, [groceryProducts.length, isPaused]);
