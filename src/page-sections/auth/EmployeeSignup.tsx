...
//          >
//           <Icon variant="small">{showPassword ? "eye-alt" : "eye"}</Icon>
//          </IconButton>
//         }
//        />
//       )}
...
//         Password must be 9+ chars with uppercase, lowercase & special character
//        </span>
//       </FlexBox>
//      )}
...
//          >
//           <Icon variant="small">{showConfirmPassword ? "eye-alt" : "eye"}</Icon>
//          </IconButton>
//         }
//        />
//       )}
    ...
    useEffect(() => {
        const fetchEmployeeDetails = async () => {
            if (phone && /^01[0-9]{9}$/.test(phone)) {
                try {
                    const response = await axios.post(`v1/employee-details`, { phone });
                    const result = response.data;

                    if (result.success && result.data) {
                        const empData = result.data;
                        if (empData.name) setValue("name", empData.name);
                        if (empData.email) setValue("email", empData.email);
                        if (empData.salary) setValue("salary", empData.salary);
                        if (empData.company_name) setValue("company_name", empData.company_name);
                        if (empData.employee_id) setValue("employee_id", empData.employee_id);
                        if (empData.designation) setValue("designation", empData.designation);
                        if (empData.nid) setValue("nid", empData.nid);

                        if (empData.seller_id) {
                            setValue("corporate_shop_id", empData.seller_id.toString());
                        }

                        toast.success("Employee details auto-filled successfully!", {
                            position: "bottom-right",
                        });
                    }
                } catch (error: any) {
                    console.error("Failed to auto-fill employee details:", error);
                }
            }
        };
