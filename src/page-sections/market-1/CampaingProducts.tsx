...
interface CampaignProductsProps {
 // Optional prop to specify which campaign type to show
 campaignType?: string;
 // Optional prop to limit number of products shown
 maxProducts?: number;
}
...
export default function CampaignProducts({
  ...
  const fetchProducts = async () => {
   try {
    ...
    if (response.data && response.data.data) {
     ...
     // Check if campaign type matches (if specified)
     if (campaignType && fetchedCampaign?.type !== campaignType) {
      console.warn(
       `Campaign type mismatch. Expected: ${campaignType}, Got: ${fetchedCampaign?.type}`
      );
      // You can choose to not display anything or show a message
      setCampaignProducts([]);
     ...
     console.log("campaignProducts", response.data.data);
     console.log("campaignInfo", fetchedCampaign);
    } else {
     console.error("Unexpected response format:", response.data);
    }
   } catch (error) {
 ...
 const getSeeMoreLink = () => {
  ...
  return `/campaign/campaign?type=${campaignInfo.slug}`;
 };

 // Don't render anything if no campaign info or products
 if (!campaignInfo || displayProducts.length === 0) {
  return null;
 }
