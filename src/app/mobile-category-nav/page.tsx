  ...
  useEffect(() => {
    const fetchNavigations = async () => {
      try {
        ...
        const data = await response.json();
        setNavigations(data); // Set the fetched categories to state
  
        // Automatically select the first category if available
        if (data.length > 0) {
          setCategory(data[0]); // Set the first category as the selected category
          setSubCategoryList(data[0]?.menuData?.categories || []); // Set the subcategories of the first category
        }
      } catch (error) {
        setError(error.message);
      } finally {
