     ...
     {selectedSpec && (
      <Box style={{ paddingTop: "8px", paddingBottom: "8px" }}>
       <SemiSpan
        style={{ fontWeight: "bold", color: "#000", fontSize: "15px" }}
       >
        {Attributes?.find((attr) => attr.Vid === selectedSpec)?.PropertyName}:
        &nbsp;
        {configuredItems
         .find(
          (item) =>
             ...
             {item.QuantityRanges?.length ? (
                ...
                const activeTier = pricingTiers?.find(
                 (tier) =>
                  ...
                  (!pricingTiers[pricingTiers.indexOf(tier) + 1] ||
                   currentQuantity <
                    pricingTiers[pricingTiers.indexOf(tier) + 1].MinQuantity)
                );
              ...
              <td className={styles.tableCellStyle}>
               {(() => {
                ...
                const applicableTier = item.QuantityRanges.slice() // Create a copy
