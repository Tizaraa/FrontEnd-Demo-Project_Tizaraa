...
export const renderProductCount = (
  ...
  if (endNumber > totalProductLength) endNumber = totalProductLength;

  return `Showing ${startNumber + 1}-${endNumber} of ${totalProductLength} products`;
};
 ...
 * @param  price - PRODUCT PRICE
 * @param  fraction - HOW MANY FRACTION DIGITS TO SHOW
 * @param  currencyCode - CURRENCY CODE (DEFAULT IS 'BDT')
 * @returns - RETURN PRICE WITH CURRENCY
 */
export function currency(
  price: number,
  fraction: number = 2,
  currencyCode: string = "BDT" // default is BDT for Bangladeshi Taka
) {
