...
const ProductFilterCard: React.FC<ProductFilterCardProps> = ({
  ...
  const sortCategoriesAlphabetically = (categories: Category[]): Category[] => {
    return categories
      .map(category => ({
        ...category,
        child: category.child ? sortCategoriesAlphabetically(category.child) : undefined
      }))
      .sort((a, b) => a.categorie_name.localeCompare(b.categorie_name, undefined, { sensitivity: 'base' }));
  };
      ...
      {items.map((item) => (
        <React.Fragment key={item.id}>
          <FilterButton
            ...
            selectedItems={[]}
            onClick={(event) =>
              handleCategoryClick(item.categorie_name_slug, event)
            }
            labelKey="categorie_name"
            isCategory={true}
          />
